...
def download_laion_dataset():
    ...
    if len([f for f in os.listdir(settings.LAION_DOWNLOAD_DIR) if f.endswith('.parquet')]) <= 1:
...
def download_diffusiondb_dataset():
    ds = load_dataset('poloclub/diffusiondb', settings.DIFFUSIONDB_SUBSET)['train']
    ds.to_parquet(settings.DIFFUSIONDB_DOWNLOAD_PATH)


def preprocess_laion_dataset():
    dfs = [pd.read_parquet(settings.LAION_DOWNLOAD_DIR + f) for f in os.listdir(settings.LAION_DOWNLOAD_DIR) if f.endswith('.parquet')]
    df = pd.concat(dfs)
    df = df[['jpg']]
    df.rename(columns={'jpg': 'image'})
